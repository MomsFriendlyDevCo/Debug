...
export default function DebugFactory(prefix) {
	...
	*/
	...
	/**
	* Utilify function to flatten Objects into something we can print
	* This mainly fixes up Vue proxies into a native object
	...
	*/
	debug.deconstruct = function(v) {
		let vType = typeof v;
		if (vType == 'object') { // Try to flatten objects
			try {
				return JSON.parse(JSON.stringify(v));
			} catch (e) { // Something stupid like circular references
				console.log('SCALAR', v);
				return v;
			}
		} else if (vType == 'string') { // Simple string output - Apply highlights
			return debug._highlights.reduce((str, highlight) =>
				str.replace(highlight.match, (all, ...matches) =>
					highlight.prefix
					+ (
						matches.length > 1
						? highlight.color(...matches) // Some match groups, pass in order to function
						: highlight.color(all) // No match groups, assume entire expression
					)
					+ highlight.suffix
				)
			, v);
		} else { // Raw print other weirdness
			return v;
		}
	};
	...
	*/
	...
	/**
	* Quick method to return a new instance of a debugger but carry over all active options like highlighting
	* @param {string|VueComponent} prefix Optional prefix to set for the new instance
	* @returns {Debug} New debugging instance
	*/
	debug.clone = function(prefix) {
		let newClone = DebugFactory(prefix || this._prefix);
		['_method', '_drain', '_maxVerbosity', 'colors', 'enabled', '_highlights'].forEach(k =>
			newClone[k] = this[k]
		);
	...
	// Highlighting {{{
	/**
	* Specify highlights to apply to various RegEx matches
	* These are processed in order
	* @type {array} A collection of highlights to be processed
	* @property {RegExp} match RegEx expression to match to highlight
	* @property {function} highlight Function to enclose the match within - usually a pointer to `colors.*`, if capture groups are used they are passed as the arguments
	* @property {string} [prefix] Optional prefix
	* @property {string} [suffix] Optional suffix
	*/
	debug._highlights = [];


	/**
	* Add a highlight to a RegEx match
	* @param {RegExp} match The highlight to match
	* @param {function} color Function to enclose the match within - usually a pointer to `colors.*`, if capture groups are used they are passed as the arguments
	* @param {Object} [options] Additional options
	* @param {string} [options.prefix] Optional prefix
	* @param {string} [options.suffix] Optional prefix
	* @returns {Debug} New debugging instance
	*/
	debug.highlight = function(match, color, options) {
		if (!(match instanceof RegExp)) throw new Error('Match must be a RegExp');
		if (typeof color != 'function') throw new Error('Color must be a function');

		debug._highlights.push({
			match,
			color,
			prefix: '',
			suffix: '',
	...
	* @param {boolean} [options.flatten=true] Flatten complex objects down to path-dotted notation
	* @param {Array<string>|Set} [options.ignore=['updated']] Paths to ignore from output (if `{flatten: true}` in dotted for to avoid recursion)
	* @param {Boolean} [options.ignoreNullish=true] Ignore null->undef or undef->null changes
	...
	* @param {function} [options.highlight] Function to show the changes between two items - shows numeric diffs + string diff from start
	...
	*/
	debug.diff = function(options) {
		let settings = {
			...
			highlight(d, settings) {
				return isFinite(d.valueOld) && isFinite(d.valueNew) ? // Numeric differences
					d.valueNew > d.valueOld ? settings.inspect(d.valueNew) + ' ' + this.colors.gray('(+' + (d.valueNew-d.valueOld) + ')')
					: d.valueNew < d.valueOld ? settings.inspect(d.valueNew) + ' ' + this.colors.gray('(' + (d.valueNew-d.valueOld) + ')')
					: settings.inspect(d.valueNew)
				: typeof d.valueOld == 'string' && typeof d.valueNew == 'string' ? (()=> { // String differences
					let o = 0;
					for (o = 0; o < Math.min(d.valueNew.length, d.valueOld.length); o++) {
						if (d.valueNew.at(o) != d.valueOld.at(o)) break;
					}

					return this.colors.gray('"')
						+ this.colors.dim(d.valueNew.substr(0, o))
						+ this.colors.underline(d.valueNew.substr(o))
						+ this.colors.gray('"')
				})()
				: settings.inspect(d.valueNew)
			},
			formatter(diff) {
				return '\n' + diff
					.map(d =>
						d.change == 'changed' ? ['  •', this.colors.cyan(`${d.key}:`), settings.inspect(d.valueOld, settings), '=>', settings.highlight.call(this, d, settings)]
						: d.change == 'new' ? ['  •', this.colors.cyan(`${d.key}:`), this.colors.gray('[undef]'), '=>', settings.highlight.call(this, d, settings), this.colors.gray('(new)')]
						: null
					)
					.filter(Boolean)
					.map(d => d.join(' '))
			...
			// flattenDiff {{{
			flattenDiff: (original, updated, path, settings) => {
				let output = [];
				if (updated === null || updated === undefined) return output; // Skip tracking meta types

				Object.keys(updated)
					.map(key => {
						if (
							settings.ignore.has(path.concat([key]))
							|| _.isEqual(original?.[key], updated?.[key]) // Already equal
						) return; // In ignore list or already equal

						if (typeof updated?.[key] != 'object') { // Scalar change
							output.push({
						...
						} else if (typeof updated?.[key] == 'object' && path.length + 1 < settings.flattenDepth) { // Within depth range and is an object
							output = output.concat(
								settings.flattenDiff(original[key], updated[key], path.concat([key]), settings)
							)
						}
					})
				return output;
		...
		*/
		...
		Object.keys(updated)
			.forEach(key => {
				if (settings.ignore.has(key)) {
					...
					settings.ignoreNullish
					&& (original[key] === null || original[key] === undefined)
					&& (updated[key] === null || updated[key] === undefined)
				) {
					// Pass - we're ignoring nullish changes
					...
					if (settings.flatten && typeof updated[key] == 'object') {
						diffs = diffs.concat(
							settings.flattenDiff(original[key], updated[key], [key], settings)
						)
					} else {
						diffs.push({
							key,
		...
		if (diffs.length > 0) {
			settings.log(settings.prefix, settings.formatter.call(this, diffs, settings));
