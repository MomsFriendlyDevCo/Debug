	...
	debug.diff = function(options) {
		let settings = {
			...
			formatter(diff) {
				return '\n' + diff
					.map(d =>
						d.change == 'changed' ? ['  •', this.colors.cyan(`${d.key}:`), settings.inspect(d.valueOld, settings), '=>', settings.highlight(d, settings)]
						: d.change == 'new' ? ['  •', this.colors.cyan(`${d.key}:`), this.colors.gray('[undef]'), '=>', settings.highlight(d, settings), this.log.colors.gray('(new)')]
						: null
					)
					.filter(Boolean)
					.map(d => d.join(' '))
