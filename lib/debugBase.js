...
export default function DebugFactory(prefix) {
	...
	*/
	debug.clone = function(prefix) {
		...
		['_method', '_drain', '_maxVerbosity', 'enabled', '_highlights'].forEach(k =>
			newClone[k] = this[k]
		);
	...
	debug.diff = function(options) {
		let settings = {
			...
			highlight: (d, settings) =>
				isFinite(d.valueOld) && isFinite(d.valueNew) ? // Numeric differences
					d.valueNew > d.valueOld ? settings.inspect(d.valueNew) + ' ' + settings.log.colors.gray('(+' + (d.valueNew-d.valueOld) + ')')
					: d.valueNew < d.valueOld ? settings.inspect(d.valueNew) + ' ' + settings.log.colors.gray('(' + (d.valueNew-d.valueOld) + ')')
				...
				: typeof d.valueOld == 'string' && typeof d.valueNew == 'string' ? (()=> { // String differences
					...
					return settings.log.colors.gray('"')
						+ settings.log.colors.dim(d.valueNew.substr(0, o))
						+ settings.log.colors.underline(d.valueNew.substr(o))
						+ settings.log.colors.gray('"')
				})()
				: settings.inspect(d.valueNew),
			formatter: (diff) => '\n' + diff
				.map(d =>
					d.change == 'changed' ? ['  •', settings.log.colors.cyan(`${d.key}:`), settings.inspect(d.valueOld, settings), '=>', settings.highlight(d, settings)]
					: d.change == 'new' ? ['  •', settings.log.colors.cyan(`${d.key}:`), settings.log.colors.gray('[undef]'), '=>', settings.highlight(d, settings), settings.log.colors.gray('(new)')]
					: null
				)
				.filter(Boolean)
				.map(d => d.join(' '))
